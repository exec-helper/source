        ...
        namespace baseUtils {
            ...
            class Statement {
                public:      
                    ...
                    virtual void write(gsl::not_null<YamlWriter*> yaml, const std::string& command) const noexcept = 0;

                    virtual inline unsigned int getNumberOfExecutions() const noexcept {
                        return m_execution.getNumberOfExecutions();
                    }

                    virtual inline void resetExecutions() noexcept {
                        m_execution.clear();
                    }

                protected:
                    Statement(ReturnCode returnCode) noexcept :
                        m_execution(returnCode)
                    {
