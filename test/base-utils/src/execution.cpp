...
namespace {
    void childProcessExecute(const CommandLineRaw& line, const Path& workingDir) noexcept {
        ReturnCode returnCode;
        
        error_code error;
        current_path(workingDir, error);
        assert(error == boost::system::errc::success); // Either works or fix the test
...
namespace execHelper {
    namespace test {
        namespace baseUtils {
            namespace execution {
                int execute(const CommandLineRaw& commandLine, const Path& workingDir) noexcept {
                    pid_t pid = fork();
                    assert(pid != -1);
                    if(pid == 0) {
