...
struct ConsoleListener : Catch::TestEventListenerBase {
    public:
        explicit ConsoleListener(const Catch::ReporterConfig& config) noexcept :
            ...
            m_stream(std::cout.rdbuf()), // Duplicate the current cout backend (should stil be stdout at this point) to a separate stream, as std::cout itself can be taken over by a reporter
