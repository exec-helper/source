...
namespace execHelper { namespace test { namespace utils {
   ...
   string convertToConfig(const vector<string>& keys, const vector<string>& values, const string& prepend) noexcept {
    ...
    string convertToConfig(const Patterns& patterns) noexcept {
        ...
        if(!patterns.empty()) { 
            ...
            for(auto& pattern : patterns) {
                ...
                if(shortOption) {
                    stream << convertToConfig("short-option", string(1U, shortOption.get()), patternNestedCharacter);
                }
                const auto longOption = pattern.getLongOption();
                if(longOption) {
                    stream << convertToConfig("long-option", longOption.get(), patternNestedCharacter);
                }
                stream << convertToConfig("values", pattern.getValues(), patternNestedCharacter);
            }
        }
        return stream.str();
    }
    ...
    void writeSettingsFile(not_null<ConfigFileWriter*> configFileWriter, const SettingsNode& settings, const Patterns& patterns) noexcept {
    ...
    PatternPermutator makePatternPermutator(const Patterns& patterns) noexcept {
        ...
        for(const auto& pattern : patterns) {
            patternValuesMatrix.emplace(make_pair(pattern.getKey(), pattern.getValues()));
        }
        return PatternPermutator(patternValuesMatrix);
    }

    ExecutorStub::TaskQueue getExpectedTasks(const Task& task, const Patterns patterns) noexcept {
    ...
    ExecutorStub::TaskQueue getExpectedTasks(const ExecutorStub::TaskQueue& tasks, const Patterns patterns) noexcept {
        ExecutorStub::TaskQueue result;
        for(const auto& combination : makePatternPermutator(patterns)) {
