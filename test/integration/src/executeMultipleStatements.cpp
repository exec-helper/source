...
namespace integration {
    ...
    SCENARIO("Execution order of a command consisting of one predefined statement", "[execute-multiple-statements][execute-multiple-statements-predefined-order][scenario-execute-multiple-statements-predefined-order-single]") {
        GIVEN("A valid configuration with multiple commands consisting of one predefined, valid, successful statement") {
            const vector<string> commandStrings({"command1", "command2", "command3", "command4"});
            ConfigBuilder execution;
            ConfigFileWriter config;
            YamlWriter yaml;

            for(const auto& commandString : commandStrings) {
                execution.add(TestCommand(commandString, {createStatement<SimpleStatement>(SUCCESS)}));
            }
            ...
            WHEN("we call each of these commands separately") {
                for(const auto& command : execution) {
                    ...
                    const ReturnCode returnCode = execute({EXEC_HELPER_BINARY, command.get(), "--settings-file", config.getFilename()}, config.getDirectory());
                    THEN_CHECK("the call should succeed") {
                        REQUIRE(returnCode == SUCCESS);
                    }

                    THEN_CHECK("the associated predefined statement should be called exactly once") {
                    ...
                    THEN_CHECK("no other predefined statements should have been called") {
        ...
        GIVEN("A valid configuration with multiple commands consisting of multiple, predefined, valid, successful statements") {
            ...
            for(const auto& commandString : commandStrings) {
                ...
                for(auto i = 0U; i < NB_OF_STATEMENTS; ++i) {
                    string statementKey = string(commandString).append("-statement").append(to_string(i));
                    auto indirect = make_shared<IndirectStatement>(statementKey, Statements({createStatement<SimpleStatement>(SUCCESS)}));
            ...
            WHEN("we call each of these commands separately") {
                for(const auto& command : execution) {
                    ...
                    THEN_CHECK("the associated predefined statements should be called the expected number of times in the right order") {
                        REQUIRE(command.getNbOfStatements() == NB_OF_STATEMENTS);
                        REQUIRE(command.getNumberOfStatementExecutions() == NB_OF_STATEMENTS);
                        for(const auto& statement : command) {
                            REQUIRE(statement->getNumberOfExecutions() == 1U);
                        }
                    }

                    THEN_CHECK("no other predefined statements except should have been called") {
                        for(const auto& otherCommand : execution) {
                            if(command.get() != otherCommand.get()) {
                                for(const auto& statement : otherCommand) {
                                    REQUIRE(statement->getNumberOfExecutions() == 0U);
    ...
    SCENARIO("Duplicate configured statements", "[execute-multiple-statements][scenario-execute-multiple-statements-predefined-order-duplicates]") {
        GIVEN("A valid configuration with a command consisting of multiple, duplicate statements and some non-duplicate ones") {
            ...
            auto duplicateStatement = make_shared<IndirectStatement>(duplicateStatementString, Statements({createStatement<SimpleStatement>(SUCCESS)}));
            TestCommand commandWithDuplicates(duplicateCommandString, Statements(NB_OF_DUPLICATE_STATEMENTS, duplicateStatement));

            // Add non-duplicate ones
            const auto NB_OF_UNIQUE_STATEMENTS = 3U;
            for(auto i = 0U; i < NB_OF_UNIQUE_STATEMENTS; ++i) {
                string statementKey = string(duplicateCommandString).append("-statement").append(to_string(i));
                auto indirect = make_shared<IndirectStatement>(statementKey, Statements({createStatement<SimpleStatement>(SUCCESS)}));
            ...
            WHEN("we call this command") {
                ...
                const ReturnCode returnCode = execute({EXEC_HELPER_BINARY, commandWithDuplicates.get(), "--settings-file", config.getFilename()}, config.getDirectory());
                ...
                THEN("the associated predefined statements should all be called the expected number of times") {
                    REQUIRE(commandWithDuplicates.getNbOfStatements() == NB_OF_UNIQUE_STATEMENTS + NB_OF_DUPLICATE_STATEMENTS);
                    for(const auto& statement : commandWithDuplicates) {
                        ...
                        REQUIRE(statement->getNumberOfExecutions() == expectedNumberOfExecutions);
                    }
                }
    ...
    SCENARIO("Execute multiple statements: predefined order: failing statements", "[execute-multiple-statements][scenario-execute-multiple-statements-predefined-order-failing-statements]") {
        GIVEN("A valid configuration with a command consisting of multiple, predefined, valid, successful statements and a failing statement that is not the first nor the last statement") {
            const vector<string> otherCommands({"other-command1", "other-command2", "other-command3"});
            ...
            ensures(NB_OF_STATEMENTS >= 3);   // In order to have a failing command that is not the first nor the last statement, a statement size of at least 3 statements is required
            mt19937 gen(0);
            uniform_int_distribution<> dis(1, NB_OF_STATEMENTS - 2U);    // Find a random index that will fail, that is not the first nor the last one
            ...
            for(uint8_t i = 0; i < NB_OF_STATEMENTS; ++i) {
                if(i == failIndex) {
                    string statementKey = string(commandString).append("-statement").append(to_string(i));
                    auto indirect = make_shared<IndirectStatement>(statementKey, Statements({createStatement<SimpleStatement>(expectedReturnCode)}));
            ...
            WHEN("we call this command") {
                auto iterator = execution.startIteration();
                const ReturnCode returnCode = execute({EXEC_HELPER_BINARY, failingCommand.get(), "--settings-file", config.getFilename()}, config.getDirectory());
                ...
                THEN("the statements configured before the failed statement and the failed statements should have been executed") {
                    REQUIRE(failingCommand.getNumberOfStatementExecutions() == failIndex + 1U);
                    auto expectedNumberOfExecutions = 1U;
