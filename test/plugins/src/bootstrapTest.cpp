...
namespace execHelper { namespace plugins { namespace test {
    ...
    SCENARIO("Obtaining the default variables map of the bootstrap plugin", "[bootstrap]") {
    ...
    SCENARIO("Test the combination of several settings for the bootstrap plugin", "[bootstrap]") {
        MAKE_COMBINATIONS("Of several settings") {
            ...
            COMBINATIONS("Set a build directory") {
                buildDir = "{" + pattern1.getKey() + "}/{" + pattern2.getKey() + "}/{HELLO}/{" + pattern2.getKey() + "}/hello{" + pattern1.getKey() + " }world"; 
