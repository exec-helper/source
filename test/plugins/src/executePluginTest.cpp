...
namespace {
    ...
    struct Expected {
        ...
        explicit Expected(Command directCommand, Task task = Task(), VariablesMap variablesMap = VariablesMap(MEMORY_KEY), Patterns patterns = Patterns()) :
...
namespace execHelper { namespace plugins { namespace test {
    ...
    SCENARIO("Obtaining the default variables map of the execute-plugin", "[clang-tidy]") {
    ...
    SCENARIO("Testing the default execute settings", "[execute-plugin]") {
        GIVEN("A selector plugin object and the default options") {
            ...
            WHEN("We apply the selector plugin") {
                bool success = plugin.apply(task, VariablesMap(PLUGIN_NAME), Patterns());
    ...
    SCENARIO("Test the settings node to variables map mapping", "[execute-plugin]") {
        MAKE_COMBINATIONS("Of settings node configurations") {
            ...
            COMBINATIONS("Add an indirect command") {
                ...
                const vector<string> directCommands({"direct-command1", "direct-command2"});

                settings.add(COMMAND_KEY, command);
                settings.add(command, directCommands);
                for(const auto& directCommand : directCommands) {
                    settings.add(directCommand, MEMORY_KEY);
                    expected.emplace(command, vector<Expected>({Expected(directCommand)}));
                }
            }
            ...
            COMBINATIONS("Add some root settings") {
                for(const auto& command : commands) {
                    ...
                    for(auto& expectedTask : expected.at(command)) {
                        ...
                        expectedVariableMap.add(string(directCommand).append("-root-setting1"), "root-setting-value1");
                        expectedVariableMap.add(string(directCommand).append("-root-setting2"), "root-setting-value2");
                        expectedVariableMap.add(string(directCommand).append("-root-setting3"), "root-setting-value3");
            ...
            COMBINATIONS("Add some command specific settings") {
                for(const auto& command : commands) {
                    ...
                    for(auto& expectedTask : expected.at(command)) {
                        ...
                        VariablesMap expectedVariableMap(MEMORY_KEY);
                        expectedVariableMap.add(string(directCommand).append("-specific-setting1"), "specific-setting-value1");
                        expectedVariableMap.add(string(directCommand).append("-specific-setting2"), "specific-setting-value2");
                        expectedVariableMap.add(string(directCommand).append("-specific-setting3"), "specific-setting-value3");

                        for(const auto& key : expectedVariableMap.values()) {
                            settings[MEMORY_KEY][directCommand][key] = expectedVariableMap[key];
                        }
                        expectedTask.variablesMap = expectedVariableMap;
                    }
                }
            ...
            COMBINATIONS("Add a specific pattern") {
                if(! commands.empty()) {
                    ...
                    for(auto& expectedCommand : expected.at(command)) {
                        ...
                        for(const auto& pattern : patterns) {
                        configuredPatterns.push_back(pattern);
                        settings.add({MEMORY_KEY, expectedCommand.directCommand, PATTERN_KEY}, pattern.getKey());
                            expectedCommand.patterns.push_back(pattern);
                        }
                    }
                }
            ...
            THEN_WHEN("We apply the execute plugin") {
                Task task;
                bool returnCode = plugin.apply(task, VariablesMap("random-thing"), Patterns());
                ...
                THEN_CHECK("It called the right commands") {
                    ...
                    for(const auto& command : commands) {
                        ...
                        for(const auto& expectedCommand : expected.at(command)) {
                            ...
                            for(const auto& key : expectedCommand.variablesMap.values()) {
                                REQUIRE(memory->variables.contains(key));
                                REQUIRE(memory->variables[key] == expectedCommand.variablesMap[key]);
                            }
                            ++memory;
                        }
                    }
    ...
    SCENARIO("Test problematic cases", "[execute-plugin]") {
        GIVEN("A plugin with a non-existing plugin to execute") {
            ...
            WHEN("We execute the plugin") {
                Task task;
                bool returnCode = plugin.apply(task, VariablesMap("test"), Patterns());
