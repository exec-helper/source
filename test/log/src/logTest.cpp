...
namespace {
    ...
    struct LogMessage {
        ...
        LogMessage(string date, string time, Channel channel, LogLevel level, string file, unsigned int lineNumber, string message) :
            date(move(date)),
            time(move(time)),
    ...
    LogMessage toMessage(const string& message) {
        sregex regex = sregex::compile(R"((?P<date>\S*)\s(?P<time>\S*)\s<(?P<severity>\S*)>\s\[(?P<channel>\S*)\]\s(?P<file>.*):(?P<lineNumber>\d*)\s(?P<message>.*)\s$)");
        smatch parsedResult;
        if(regex_search(message, parsedResult, regex)) {
            string lineNumberString = parsedResult["lineNumber"];       // Using lexical_cast to a non-string type on the match type directly results in undefined behaviour.
                                                                        // so convert it to an intermediate string first.
            return LogMessage(
                    parsedResult["date"],
                    parsedResult["time"],
                    parsedResult["channel"],
                    toLogLevel(parsedResult["severity"]),
                    parsedResult["file"],
    ...
    LogLevel getRelativeLogLevel(LogLevel currentLogLevel, int relativeLevel) noexcept {
        ...
        GIVEN("A list of severities") {
            for(auto severity : getLogLevels()) {
                ...
                WHEN("We switch on the right severity") {
                    ...
                    execHelper::log::setSeverity("other-channel", getRelativeLogLevel(severity, 1));
        ...
        GIVEN("A list of severities") {
            for(auto severity : getLogLevels()) {
                ...
                WHEN("We disable the right severity") {
                    ...
                    execHelper::log::init(logStream);
                    execHelper::log::setSeverity(LOG_CHANNEL, getRelativeLogLevel(severity, 1));
