        ...
        namespace test {
            SCENARIO("Test envp construction", "[config][envp]") {
                GIVEN("A taskcollection to take the arguments from") {
                    const EnvironmentCollection env({{"key1", "value1"}, {"key2", "value2"}, {"key3", "value3"}});
                    vector<string> expectedEnv;
                    expectedEnv.reserve(env.size());
                    for(const auto& element : env) {
                        expectedEnv.emplace_back(string(element.first).append("=").append(element.second));
                    }
                    ...
                    WHEN("We create the envp") {
                        ...
                        THEN("The associated envp char array should be returned") {
                            ...
                            size_t index = 0U;
                            while(returnedEnvp[index] != nullptr) {     // NOLINT(cppcoreguidelines-pro-bounds-pointer-arithmetic)
                                REQUIRE(index < env.size());
                                REQUIRE(returnedEnvp[index] == expectedEnv[index]); // NOLINT(cppcoreguidelines-pro-bounds-pointer-arithmetic)
                                ++index;
                            }
                        }
                    }
            ...
            SCENARIO("Test the envp copy and move constructor, assignment operators and the swap operator", "[config][envp][check]") {
                GIVEN("An envp object to copy") {
                    ...
                    WHEN("We copy the given object") {
                        ...
                        size_t index = 0U;
                        while(returnedEnvp[index] != nullptr) { // NOLINT(cppcoreguidelines-pro-bounds-pointer-arithmetic)
                            envpAddresses.push_back(returnedEnvp[index]);   // NOLINT(cppcoreguidelines-pro-bounds-pointer-arithmetic)
                            ++index;
                        }
                        ...
                        THEN("The values of the pointers must be equal") {
                            ...
                            size_t index = 0U;
                            while(copiedEnvp[index] != nullptr) {   // NOLINT(cppcoreguidelines-pro-bounds-pointer-arithmetic)
                                REQUIRE(expectedEnv[index] == copiedEnvp[index]);   // NOLINT(cppcoreguidelines-pro-bounds-pointer-arithmetic)
                                ++index;
                            }
                            REQUIRE(index == expectedEnv.size());
                        }
                        ...
                        THEN("It must have been a deep copy/the actual pointer must be different") {
                            ...
                            size_t index = 0U;
                            while(copiedEnvp[index] != nullptr) {   // NOLINT(cppcoreguidelines-pro-bounds-pointer-arithmetic)
                                REQUIRE(envpAddresses[index] != copiedEnvp[index]); // NOLINT(cppcoreguidelines-pro-bounds-pointer-arithmetic)
                                ++index;
                            }
                            REQUIRE(index == expectedEnv.size());
                        }
                    ...
                    WHEN("We copy assign the given object") {
                        Envp assign(EnvironmentCollection({{"copy-assign-key1", "copy-assign1"}, {"copy-assign-key2", "copy-assign2"}}));
                    ...
                    WHEN("We move the given object") {
                        ...
                        THEN("We must find the expected content") {
                            ...
                            size_t index = 0U;
                            while(movedEnvp[index] != nullptr) {    // NOLINT(cppcoreguidelines-pro-bounds-pointer-arithmetic)
                                REQUIRE(expectedEnv[index] == movedEnvp[index]);    // NOLINT(cppcoreguidelines-pro-bounds-pointer-arithmetic)
                                ++index;
                            }
                            REQUIRE(index == expectedEnv.size());
                        }
                    ...
                    WHEN("We move assign the given object") {
                        Envp assign(EnvironmentCollection({{"move-assign-key1", "move-assign1"}, {"move-assign-key2", "move-assign2"}}));
                    ...
                    WHEN("We swap the given object") {
                        const EnvironmentCollection swappedContent({{"swap-key1", "swap-value1"}, {"swap-key2", "swap-value2"}, {"swap-key3", "swap-value3"}});
                        vector<string> expectedSwapEnv;
                        expectedSwapEnv.reserve(swappedContent.size());
                        for(const auto& element : swappedContent) {
                            expectedSwapEnv.emplace_back(string(element.first).append("=").append(element.second));
                        }
                        ...
                        THEN("We must find the expected content for the first object") {
                            ...
                            size_t index = 0U;
                            while(swappedEnvp[index] != nullptr) {  // NOLINT(cppcoreguidelines-pro-bounds-pointer-arithmetic)
                                REQUIRE(swappedEnvp[index] == expectedSwapEnv[index]);  // NOLINT(cppcoreguidelines-pro-bounds-pointer-arithmetic)
                                ++index;
                            }
                        ...
                        THEN("We must find the swapped content for the second object") {
                            ...
                            size_t index = 0U;
                            while(swappedEnvp[index] != nullptr) {  // NOLINT(cppcoreguidelines-pro-bounds-pointer-arithmetic)
                                REQUIRE(swappedEnvp[index] == expectedEnv[index]);  // NOLINT(cppcoreguidelines-pro-bounds-pointer-arithmetic)
                                ++index;
                            }
                            REQUIRE(index == expectedEnv.size());
                        }
            ...
            SCENARIO("Test the envp streaming operator", "[config][envp]") {
                GIVEN("An envp object to stream") {
                    const EnvironmentCollection env({{"stream-key1", "stream1"}, {"stream-key2", "stream2"}, {"stream-key3", "stream3"}});
                    const Envp envp(env);
