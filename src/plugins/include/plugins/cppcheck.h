...
class Cppcheck : public Plugin {
  ...
  private:
     ...
     * \param[in] rootSettings  The configuration settings associated with the
     * specific command \returns The source dir to use
     */
    static std::string
    getSourceDir(const config::Command& command,
     ...
     */
    static std::string
    getEnabledChecks(const config::VariablesMap& variables) noexcept;
};
} // namespace plugins
} // namespace execHelper
