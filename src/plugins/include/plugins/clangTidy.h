...
class ClangTidy : public Plugin {
  ...
  private:
     ...
     * @returns The configured checks as a clang-tidy check command line
     * argument
     */
    static core::TaskCollection getChecks(const Checks& checks) noexcept;

    /**
     * Returns the configured warning-as-errors as a clang-tidy check command
     * line argument
     *
     ...
     * line argument
     */
    static core::TaskCollection
    getWarningAsError(const WarningAsError& warningAsError,
     ...
     * @returns The configured collection as a clang-tidy command line style
     * string
     */
