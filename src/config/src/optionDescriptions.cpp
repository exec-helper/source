...
namespace execHelper { namespace config {
    ...
    bool OptionDescriptions::setPositionalArgument(const OptionInterface& option) noexcept {
        m_positional = option.getId();
        return true;
    }

    bool OptionDescriptions::getOptionsMap(VariablesMap& variablesMap, const Argv& argv, bool allowUnregistered) const noexcept {
        variables_map optionsMap;
        auto commandLineParser = command_line_parser(argv.getArgc(), argv.getArgv());
        ...
        try {
            ...
            user_feedback_error("Could not parse command line arguments: " << e.what());
            return false;
    ...
    void OptionDescriptions::toMap(VariablesMap& variablesMap, const variables_map& optionsMap) const noexcept {
