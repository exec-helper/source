...
namespace {
    ...
    bool processPattern(not_null<VariablesMap*> currentSettings, const SettingsNode& newSettings) noexcept {
    ...
    Patterns processPatterns(const SettingsNode& settings) noexcept {
        ...
        if(settings.contains(PATTERNS_KEY)) {
            ...
            for(const auto& patternKey : patternSettings.values()) {
                ...
                auto newPattern = PatternsHandler::toPattern(patternKey, newPatternMap);
                if(! newPattern) {
                    continue;
                }
