...
namespace execHelper {
    namespace config {
        ...
        /**
         * \brief Interface for determining the option characteristics associated with an argument option
         */
        class OptionInterface {
            public:
                 ...
                 *          False   otherwise
                 */
                virtual bool toMap(config::VariablesMap& variablesMap, const boost::program_options::variables_map& optionsMap) const noexcept = 0;

                /**
                 * Return the associated type value of this options in order to be able to properly parse it
                 *
                 * \returns The implementation-specific semantic to be used by the optionparser.
                 */
                virtual const boost::program_options::value_semantic* getTypeValue() const noexcept = 0;
        ...
        class OptionBase : public OptionInterface {
            public:
                ...
                /**
                 * Returns the argument options for this option, in addition to the one returned by getId().
                 *
                 * \returns A collection of additional option keys associated with this option
                 */
                ...
                bool toMap(config::VariablesMap& /*variablesMap*/, const boost::program_options::variables_map& /*optionsMap*/) const noexcept override {
                    ensuresMessage(false, "We should not get here: all children should implement this function");
                    return false;
                }

                const boost::program_options::value_semantic* getTypeValue() const noexcept override {
                    ensuresMessage(false, "We should not get here: all children should implement this function");
            ...
            protected:
                /**
                 * \param[in] identifyingOption The unique string to identify this option with
                 * \param[in] argumentOptions   A collection of additional, non-identifying options
                 * \param[in] explanation       The explanation associated with this option
                 */
                OptionBase(const std::string& identifyingOption, const ArgumentOptions& argumentOptions, const std::string& explanation) :
                    OptionInterface(),
                    m_identifyingOption(identifyingOption),
                    m_argumentOptions(argumentOptions),
                    m_explanation(explanation)
                {
                    expectsMessage(argumentOptions.size() <= 1U, "Currently we support only one additional, non-identifying option: the associated short option");
                }

                std::string m_identifyingOption;  //!< The identifying option associated with this option
                ArgumentOptions m_argumentOptions;    //!< Additional non-identifying options associated with this option
                std::string m_explanation;    //!< The associated explanation
        };
        typedef std::vector<OptionBase> OptionCollection;

        /**
         * \brief Class implementing an argument option that fits an option value of type T
         */
        template<typename T>
        class Option : public OptionBase {
            public:
                 ...
                 */
                Option(const std::string& identifyingOption, const ArgumentOptions& argumentOptions, const std::string& explanation) :
        ...
        /**
         * \brief   Partial implementation an argument option that fits an option value of type vector<T>
         */
        template<typename T>
        ...
        class OptionDescriptions {
            public:
                 ...
                 */
                template<typename T>
                void addOption(const Option<T>& option) noexcept {
                    ...
                    m_descriptions(option_code.c_str(), option.getTypeValue(), option.getExplanation().c_str());
                    m_options.emplace(std::pair<std::string, std::unique_ptr<Option<T>>>(id, std::make_unique<Option<T>>(option)));
                }

                /**
                 * Set the given option as the positional argument (meaning that if no associated option is specified for an argument on the command line, it is assumed to belong to this specific option.
                 *
                 ...
                 * \returns True    if the options map was successfully constructed
                 *          False   otherwise
                 */
                bool getOptionsMap(config::VariablesMap& variablesMap, const Argv& argv, bool allowUnregistered = false) const noexcept;

            private:
                void toMap(config::VariablesMap& variablesMap, const boost::program_options::variables_map& optionsMap) const noexcept;
