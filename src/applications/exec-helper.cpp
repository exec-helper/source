...
namespace {
    ...
    struct ConfigOptions {
        ConfigOptions(Path&& settingsFile, FleetingOptions&& fleetingOptions, SettingsNode&& settings, Patterns&& patterns) noexcept :
    ...
    inline void printHelp(const OptionDescriptions& options, const SettingsNode& settings) noexcept {
        ...
        if(settings.contains(COMMANDS_KEY)) {
            ...
            for(const auto& command : settings[COMMANDS_KEY].values()) {
                ...
                CommandCollection commmandDescription = settings[COMMANDS_KEY].get<CommandCollection>({command}, {});
    ...
    inline bool verifyOptions(const FleetingOptions& options) noexcept {
        if(options.getJobs() == 0U) {
            user_feedback_error("Invalid value passed for the number of jobs (0): it must be strictly positive.");
            return false;
        } 
        return true;
    }

    inline boost::optional<Path> getSettingsFile(const Argv& argv, const EnvironmentCollection& env, const Option<SettingsFileOption_t>& settingsOption) noexcept {
        ...
        if(! options.getOptionsMap(optionsMap, argv, true)) {
            user_feedback_error("Could not properly parse the command line options");
            return boost::none;
        }
        FleetingOptions fleetingOptions(optionsMap);

        ConfigFileSearcher configFileSearcher(getSearchPaths(env));
        SettingsFileOption_t settingsFileValue = optionsMap.get<SettingsFileOption_t>(SETTINGS_FILE_KEY, ".exec-helper");
        boost::optional<Path> settingsFile = configFileSearcher.find(settingsFileValue);
    ...
    inline optional<ConfigOptions> handleConfiguration(const Argv& argv, const EnvironmentCollection& env) noexcept {
        OptionDescriptions options;
        options.addOption(Option<HelpOption_t>(HELP_KEY, {"h"}, "Produce help message"));
        options.addOption(Option<VerboseOption_t>(VERBOSE_KEY, {"v"}, "Set verbosity"));
        options.addOption(Option<JobsOption_t>(JOBS_KEY, {"j"}, "Set number of jobs to use. Default: auto"));
        options.addOption(Option<DryRunOption_t>(DRY_RUN_KEY, {"n"}, "Dry run exec-helper"));
        auto settingsFileOption = Option<SettingsFileOption_t>(SETTINGS_FILE_KEY, {"s"}, "Set the settings file");
        options.addOption(settingsFileOption);
        options.addOption(Option<LogLevelOption_t>(LOG_LEVEL_KEY, {"d"}, "Set the log level"));
        auto commandOption = Option<CommandCollection>(COMMAND_KEY, {"z"}, "Commands to execute");
        ...
        for(const auto& pattern : patterns) {
            ...
            if(shortOption) {
                additionalArguments.emplace_back(ArgumentOption(1, shortOption.get()));
            }
            const auto longOption = pattern.getLongOption();
            if(longOption) {
                options.addOption(Option<PatternValues>(longOption.get(), additionalArguments, "Values for pattern '" + longOption.get() + "'"));
            }
        }
        ...
        for(auto& pattern : patterns) {
            ...
            if(longOption && optionsMap.contains(longOption.get())) {
                pattern.setValues(optionsMap.get<PatternValues>(longOption.get(), pattern.getValues()));
            }
        }

        return ConfigOptions(move(settingsFile.get()), move(fleetingOptions), move(settings), move(patterns));
    }
}
...
int execHelperMain(int argc, char** argv, char** envp) {
    ...
    execHelper::plugins::ExecuteCallback executeCallback = [executor = executor.get()](const Task& task) { executor->execute(task);};
    execHelper::plugins::registerExecuteCallback(executeCallback);

    Commander commander;
    if(commander.run(fleetingOptions, move(settings), move(patterns), settingsFile.parent_path(), move(env))) {
        return EXIT_SUCCESS;
    } else {
        user_feedback_error("Error executing commands");
        return EXIT_FAILURE;
