FROM debian:testing
RUN apt-get update && apt-get install --yes --no-install-recommends meson cmake make libboost-dev libboost-program-options-dev libboost-log-dev libyaml-cpp-dev libmsgsl-dev liblua5.3-dev pkg-config g++ git python3 catch sudo curl libreadline-dev python3-sphinx python3-sphinx-rtd-theme && apt-get clean --yes && rm -rf /var/lib/apt/lists/*
