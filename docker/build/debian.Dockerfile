FROM debian:testing
RUN apt-get update && apt-get install --yes cmake make libboost-dev libboost-program-options-dev libboost-log-dev libmsgsl-dev pkg-config g++ git doxygen python catch sudo curl && apt-get clean --yes
